...
public class PolylineMap {
    ...
    public void removerPolyline(Polyline polyline) {
        String keyToRemove = null;
        for (Map.Entry<String, PolylineWithCost> entry : polylineMap.entrySet()) {
            if (entry.getValue().getPolyline().equals(polyline)) {
                keyToRemove = entry.getKey();
                break;
            }
        }
        if (keyToRemove != null) {
            polylineMap.remove(keyToRemove);
        }
    }
}
